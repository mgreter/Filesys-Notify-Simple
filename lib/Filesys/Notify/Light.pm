...
# on cygwin we convert win to posix paths
...
{
	my($class, $path) = @_;
	Carp::croak('Usage: Filesys::Notify::Light->new([ $path1, $path2 ])')
...
sub wait_win32 {
	...
	return sub {
		...
		while(1) {
			...
			Carp::croak("Can't wait notifications, maybe " . scalar(@notify) .
				" directories exceeds limitation.") if ! defined $idx;
			if($idx > 0) {
				--$idx;
...
sub _inotify2_type
{
	...
	return "delete" if $event->IN_DELETE_SELF;
...
# compare fs states from old and new scan
# execute callback on all differences
sub _compare_fs
{
	my($old, $new, $cb) = @_;
			...
			if (!exists $new->{$dir}{$path}) {
				$cb->($path, 'delete');
					...
					( $old->{$dir}{$path}{mtime} != $new->{$dir}{$path}{mtime} ||
					  $old->{$dir}{$path}{ctime} != $new->{$dir}{$path}{ctime} ||
...
# scan all paths and return stats
sub _full_scan {
	my @paths = @_;
	...
	for my $path (@paths) {
		next unless -e $path;
		# there is a weird race-condition on windows
		# checking hradlinks via lstat and stat seems to solve
		# the issue (File::find also does this internally)
		# only seems to happen with Win32::ChangeNotify
		# not sure if it is purely a timing issue
		my @stat = stat $path;
		my @lstat = lstat $path;
		next unless $stat[3];
		next unless $lstat[3];
		File::Find::finddepth({
			wanted => sub {
				my $fullname = $File::Find::fullname ||
					Cwd::abs_path($File::Find::name);
				if (my $stat = _stat($fullname)) {
					# keep track of directories to emit events for them
					$map{Cwd::abs_path($path)}{$fullname} = $stat if $stat->{is_dir};
					$map{Cwd::abs_path($File::Find::dir)}{$fullname} = $stat;
				}
			},
			follow_fast => 1,
			follow_skip => 2,
...
# get object with file/dir stats
sub _stat {
	my ($path) = @_;
	return unless -e $path;
	my @stat = stat $path;
	my @lstat = lstat $path;
	die "stat1: ", @stat unless $stat[3];
	die "stat2: ", @lstat unless $lstat[3];
	return unless $stat[3];
	return unless $lstat[3];
	return {
		path => $path,
		size => $stat[7],
		mtime => $stat[9],
		ctime => $stat[10],
		is_dir => S_ISDIR($stat[2]),
	};
}
# EO _stat
...
=head1 SYNOPSIS
  ...
  $watcher->wait(sub {
	  for my $event (@_) {
		  ...
		  $event->{event}; # type of the emitted event
	  }
  });

=head1 DESCRIPTION

Filesys::Notify::Light is a simple but unified interface to get
...
Implementation was forked from L<Filesys::Notify::Simple> in 2017 and
nearly fully rewritten in 2018 to fix most known issues and to improve
event emitting to include the type of the event (create, delete or modify).
...
Directories are always watched recursively, since most observer
always report changes for the full directory tree. Filter out
events from subdirs if you don't want this behavior.

=back

=head1 AUTHOR

Marcel Greter E<lt>perl-file-notify@ocbnet.chE<gt>
...
L<File::ChangeNotify> L<Filesys::Notify::Simple> L<Mac::FSEvents>
L<Linux::Inotify2> L<Filesys::Notify::KQueue> L<Win32::ChangeNotify>
